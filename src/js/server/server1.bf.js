...
exports.conn = ((req, socket, head)=>{
  ...
  wss.on("connection", ws => {
    ...
    if(server.count < server._limit){
      //Send Client server confirmation message
      ws.send("SERVER MESSAGE: OPENED");
      //Send necessary information
      ws.send(JSON.stringify(sendInfo()), {binary:true});
      //Spawns the player
      ws.send(JSON.stringify(playerSpawn()), {binary:true});
      //Error
      ws.on("error", err =>{
      ...
      ws.on("message", msg =>{
        try{
          ...
          if(message.id === "move"){
            ...
            ws.send(JSON.stringify(playerMovement(message)), {binary:true});
          }else if(message.id === "search"){
            //Update render search list.
            staticSearchList(message);
